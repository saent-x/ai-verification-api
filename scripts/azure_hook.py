...
from cv2 import cv2k
import numpy as numpy
from util import numpy_to_cv
...
def fetchImages(prefix, connection_string, container):
    ...
    prefixed_blobs = get_blobs_with_prefix(blob_service.list_blobs(container), prefix)
...
if __name__ == "__main__":
    ...
    # cv2.imshow("selfie", numpy_to_cv(result["selfie"]))
    # cv2.imshow("id 1", numpy_to_cv(result["ids"][0]))
    # cv2.imshow("id 2", numpy_to_cv(result["ids"][1]))
